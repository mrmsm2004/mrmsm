...
//Bu kodga tegmang aks holda botingiz ishlamay qoladi
//ushbu kod @Clever_Coders @Clever_API kanali orqali tarqatildi
//Bizning kanallarimizda bundanda zo'r php kodlar bor siz ham qo'shiling
/*	F	*/
...
/*	F	*/
function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Dushanbe',$tr_num='fa'){

 $T_sec=0;/* <= رفع خطاي زمان سرور ، با اعداد '+' و '-' بر حسب ثانيه */

 if($time_zone!='local')date_default_timezone_set(($time_zone==='')?'Asia/Dushanbe':$time_zone);
 $ts=$T_sec+(($timestamp==='')?time():tr_num($timestamp));
...
/*	F	*/
function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$none='',$timezone='Asia/Dushanbe'){
...
/*	F	*/
function jgetdate($timestamp='',$none='',$timezone='Asia/Dushanbe',$tn='en'){
